...
async function main() {
    ...
    switch (command) {
        ...
        case 'build-ai': {
            ...
            // 3. LLM durumu
            const health = await (0, llmClient_1.getReasoningEngine)().checkHealth();
            if (health.available) {
        ...
        case 'hotspots': {
            ...
            const { getGitCollector } = await Promise.resolve().then(() => __importStar(require('./collectors/gitCollector')));
        ...
        case 'complexity': {
            ...
            const { ComplexityCalculator } = await Promise.resolve().then(() => __importStar(require('./parsers/complexityCalculator')));
        ...
        case 'markers': {
            ...
            const { JSDocParser } = await Promise.resolve().then(() => __importStar(require('./parsers/jsdocParser')));
            const parser = new JSDocParser();
            const fs = await Promise.resolve().then(() => __importStar(require('fs')));
            const glob = await Promise.resolve().then(() => __importStar(require('glob'))).catch(() => null);
            if (!glob) {
        ...
        case 'trends': {
            log('\n📈 Proje Trend Analizi\n', 'cyan');
            const { getTemporalMemory } = await Promise.resolve().then(() => __importStar(require('./memory/temporalMemory')));

...
import { CopilotAction, CopilotSuggestion } from '../collectors/copilotCollector';
...
export class CopilotActionParser {
   ...
   */
  private detectChangeType(action: CopilotAction): SuggestionAnalysis['changeType'] {
   ...
   */
  private extractAffectedSymbols(action: CopilotAction): SymbolInfo[] {
   ...
   * Riskleri tespit et
   */
  private detectRisks(action: CopilotAction): Risk[] {
    const risks: Risk[] = [];
    const code = action.suggestedCode;
   ...
   * Pattern çıkar
   */
  private extractPattern(action: CopilotAction): LearnedPattern | null {
    // Sadece kabul edilmiş aksiyonlardan pattern çıkar
    if (action.type !== 'accept') return null;
   ...
   */
  private categorizePattern(action: CopilotAction): string {
    const code = action.suggestedCode;

    if (/import\s/.test(code)) return 'import';
    if (/export\s/.test(code)) return 'export';
    if (/function|=>/.test(code)) return 'function';
    if (/class\s/.test(code)) return 'class';
    if (/if\s*\(|switch\s*\(/.test(code)) return 'control-flow';
    if (/try\s*{|catch\s*\(/.test(code)) return 'error-handling';
    if (/async|await|Promise/.test(code)) return 'async';
    if (/\.(map|filter|reduce|forEach)\(/.test(code)) return 'array-operation';
    if (/useState|useEffect|useRef/.test(code)) return 'react-hook';
    if (/ref\(|computed\(|watch\(/.test(code)) return 'vue-composition';
   ...
   */
  private assessComplexity(action: CopilotAction): SuggestionAnalysis['complexity'] {
    const code = action.suggestedCode;
    const lines = code.split('\n').length;
...
// Singleton
let copilotActionParser: CopilotActionParser | null = null;

export function getCopilotActionParser(): CopilotActionParser {

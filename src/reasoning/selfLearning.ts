...
export interface CommitFeedback {
  ...
  followupCommits: number; // Sonraki kaç commit'te bu dosyalara dokunuldu
  reverted: boolean;
}
...
export class SelfLearning {
  ...
  private copilotFeedbacks: CopilotFeedback[] = [];
  private commitFeedbacks: CommitFeedback[] = [];
  private preferences: Map<string, LearnedPreference> = new Map();
  private rewards: RewardSignal[] = [];
  ...
  learnFromCommits(feedback: CommitFeedback): void {
    ...
    if (feedback.impact === 'breaking') {
      for (const symbol of feedback.changedSymbols) {
        this.updatePreference(
          `symbol:${symbol}:breaking`,
          -0.5,
          `Breaking change to ${symbol}`
        );
      }
    }
    ...
    if (feedback.followupCommits > 3) {
      for (const symbol of feedback.changedSymbols) {
        this.updatePreference(
          `symbol:${symbol}:unstable`,
          -0.3,
          `Unstable change to ${symbol}`
        );
      }
    }
  }
  ...
  calculateReward(action: string, context: string, outcome: 'success' | 'failure' | 'partial'): RewardSignal {
    ...
    const signal: RewardSignal = { action, context, reward, reason };
    this.rewards.push(signal);

    // Reward'dan öğren
    this.updatePreference(
      `action:${action}:${context.split(' ')[0]}`,
      reward,
      `${action} in ${context}`
    );
   ...
   * Preference güncelle
   */
  private updatePreference(pattern: string, weight: number, example: string): void {
   ...
   */
  private extractPatterns(feedback: Feedback): string[] {
   ...
   */
  private calculateWeight(feedback: Feedback): number {
  ...
  exportPreferences(): Record<string, LearnedPreference> {
    const result: Record<string, LearnedPreference> = {};
   ...
   */
  getCopilotAcceptanceRate(): { overall: number; byContext: Map<string, number> } {
    const total = this.copilotFeedbacks.length;
    const accepted = this.copilotFeedbacks.filter(f => f.accepted).length;

    // Group by file extension
    const byContext = new Map<string, number>();
    const contextCounts = new Map<string, { accepted: number; total: number }>();
    ...
    for (const fb of this.copilotFeedbacks) {
      ...
      current.total++;
      if (fb.accepted) current.accepted++;
  ...
  getCommitStabilityScore(): number {
    if (this.commitFeedbacks.length === 0) return 0.5;

    let score = 0.5;
    
    for (const fb of this.commitFeedbacks) {
      if (fb.reverted) score -= 0.1;
      if (fb.impact === 'breaking') score -= 0.05;
      if (fb.followupCommits === 0) score += 0.02;
      if (fb.followupCommits > 3) score -= 0.03;
    }
...
// Singleton
let selfLearning: SelfLearning | null = null;

export function getSelfLearning(): SelfLearning {
  if (!selfLearning) {
